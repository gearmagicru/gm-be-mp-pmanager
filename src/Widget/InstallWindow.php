...
/**
 * Виджет формирования интерфейса окна установки плагина.
 * 
...
class InstallWindow extends Window
{
    ...
    protected function init(): void
    {
        ...
        $this->id        = 'gm-mp-pmanager-iwindow';
    ...
    /**
     * Возвращает набор полей с именем и описанием плагина в указанных локализациях.
     * 
     * @param array $names Имена и описание плагина в указанных локализациях.
     * 
     * @return array
     */
    protected function getNamesFieldset(array $params): array
    {
        $fieldset = [
            'xtype'    => 'fieldset',
            'title'    => '#Plugin name in installed locales',
            'defaults' => [
                'xtype'      => 'displayfield',
                'ui'         => 'parameter',
                'labelAlign' => 'right',
        ...
        /** @var array $translatorConfig Шаблон параметров источника (категории) транслятора плагина */
    ...
    /**
     * Формирует вкладку "Информация о плагине".
     * 
    ...
    protected function getTabInformation(array $params): array
    {
        ...
        if ($versionUrl)
            ...
            $versionUrl = '#[ unknown ]';
        // если есть уведомление об ошибке в параметрах плагина
        $notice = [];
        if ($this->notice) {
            $notice = [
                'xtype' => 'box',
        ...
        return [
            ...
            'items'      => [
                ...
                [
                    ...
                    'width' => '100%',
                    'value' => Html::tags([
                        Html::tag('div', $this->creator->t('Information about the plugin'), ['class' => 'g-form__display__text']),
                        Html::tag('div', $this->creator->t('version and author of the plugin'), ['class' => 'g-form__display__subtext'])
                    ])
                ],
                [
                    'xtype' => 'container',
        ...
        return [
            ...
            'items'      => [
                [
                    ...
                    'width' => '100%',
                    'value' => Html::tags([
                        Html::tag('div', $this->creator->t('Plugin configuration'), ['class' => 'g-form__display__text']),
                        Html::tag('div', $this->creator->t('plugin installation options'), ['class' => 'g-form__display__subtext'])
                    ])
                ],
                [
                    'xtype'    => 'container',
    ...
    {
        // добавляем шаблон параметров источника (категории) транслятора плагина
        Gm::$app->plugins->addTranslateCategory($this->info['install']['id'], $this->info['path']);
