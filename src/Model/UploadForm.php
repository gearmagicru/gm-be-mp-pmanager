...
/**
 * Модель загрузки файла пакета плагина.
 * 
...
class UploadForm extends FormModel
{
        ...
        if ($isValid) {
            ...
            if ($file->validate()) {
                ...
                if (!$this->makePackageFolder($file->getBaseName())) {
                    $this->setError(
                        $this->module->t('Error creating temporary directory to download plugin package file')
                    );
                    return false;    
                }
            } else {
    ...
    /**
     * Создаёт временный каталог плагина.
     * 
     * @param string $packageFolder Имя папки плагина.
     * 
     * @return bool
     */
    ...
    /**
     * Извлечение файлов из пакета плагина.
     * 
     * @return bool
     */
    protected function unpackProcess(): bool
    {
        ...
        if (empty($package->id) || empty($package->type)) {
            $this->setError(
                $this->module->t('The plugin package file does not contain one of the attributes: id, type')
            );
            return false;
        }
        ...
        if ($package->type !== 'plugin') {
            $this->setError(
                $this->module->t('plugin attribute "{0}" is incorrectly specified', ['type'])
            );
            return false;
        }

        /** @var array|null $params Параметры установленного плагина */
        $params = Gm::$app->plugins->getRegistry()->get($package->id);
        if ($params) {
            /** @var array|null $names */
            $names = Gm::$app->plugins->selectName($params['rowId']);
            $this->setError(
                $this->module->t(
